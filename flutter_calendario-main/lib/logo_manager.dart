  ...
  static Future<Map<String, dynamic>?> getLogo() async {
    try {
      ...
      if (base64Image != null) {
        ...
        final width = await decodeImageFromList(
          imageData,
        ).then((img) => img.width);
        final height = await decodeImageFromList(
          imageData,
        ).then((img) => img.height);
        return {
          'name': name ?? 'Logo',
...
class _LogoManagerState extends State<LogoManager> {
  ...
  Future<void> _processAndSaveLogo(Uint8List imageData) async {
    try {
      ...
      final Uint8List resizedImageData = await ImageUtils.resizeImage(
        imageData,
      );
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                icon: Icon(
                  _currentLogo != null ? Icons.edit : Icons.add_a_photo,
                ),
                label: Text(
                  _currentLogo != null ? 'Cambiar logo' : 'Agregar logo',
                ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow(
                        'Pixeles:',
                        '${_currentLogo!['width']} x ${_currentLogo!['height']}',
                      ),
                      _buildInfoRow(
                        'Tamaño:',
                        '${(_currentLogo!['size_bytes'] / 1024).toStringAsFixed(1)} KB',
                      ),
                      _buildInfoRow(
                        'Creado:',
                        _formatDate(_currentLogo!['created_at']),
                      ),
